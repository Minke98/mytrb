...
class ChatRepository extends Repository {
  ...
  static Future<Map> staticGetNewMessage(
    ...
    try {
      ...
      await dbx.transaction((db) async {
        ...
        // Gunakan BaseClient untuk melakukan request
        await BaseClient.safeApiCall(
          Environment.chatNewMsg, // Sesuaikan dengan URL API chat yang tepat
          RequestType.post,
          data: formDataMap,
          onSuccess: (response) async {
            Map newMessages = {};
            ...
            if (response.data['status'] == true &&
              ...
              if (newMessageKeys.isNotEmpty) {
                ...
                await BaseClient.safeApiCall(
                  Environment.chatUpdate,
                  RequestType.post,
                  data: formDataMapUpdate,
                  onSuccess: (response) {
                    log("== chat phase 10b update status  $response");
                  },
                  onError: (error) {
                    log("Error updating message status: ${error.message}");
                  },
                );
              }
            }
    ...
    try {
      ...
      await dbx.transaction((db) async {
        ...
        if (findChatUser.isNotEmpty) {
          ...
          // Use BaseClient to send the request
          Map<String, dynamic> formDataMap = {
            "message": message,
            "to": ucTo,
            "room": ucRoom,
          ...
          // Use BaseClient.safeApiCall
          await BaseClient.safeApiCall(
            Environment.chatSend,
            RequestType.post,
            data: formDataMap,
            onSuccess: (response) async {
  ...
  Future<Map> saveToken({String fcmtoken = ""}) async {
    ...
    try {
      ...
      if (chatUser == null) {
        ...
        // Gunakan BaseClient untuk melakukan request
        await BaseClient.safeApiCall(
          Environment.chatSaveToken, // URL untuk API
          RequestType.post, // Request Type adalah POST
          data: formDataMap, // Data yang dikirimkan dalam request
          headers: {}, // Gunakan header kosong jika perlu
          onSuccess: (response) {
  ...
  static Future<Map> joinLecturerGroup(
    ...
    try {
      ...
      // Gunakan BaseClient untuk melakukan request
      await BaseClient.safeApiCall(
        Environment.joinLecturerGroup, // URL untuk API
        RequestType.post, // Request Type adalah POST
        data: formDataMap, // Data yang dikirimkan dalam request
        headers: {}, // Header kosong jika perlu
        onSuccess: (response) async {
          if (response.data['status'] == true) {
            ...
            if (other_rooms.isNotEmpty) {
              ...
              for (var i = 0; i < other_rooms.length; i += batchSize) {
                ...
                String placeholders =
                    List.filled(batchRooms.length, '?').join(',');
                db.rawDelete(
            ...
            if (findChatUser.isEmpty) {
              await db
                  .rawInsert("INSERT INTO tech_chat_user VALUES (?,?,?,?)", [
                creator['uc_chat_user'],
        ...
        onError: (error) {
          log("Error joining lecturer group: ${error.message}");
          ret['status'] = false;
        },
      );
    } on DioError catch (e) {
  ...
  static Future<Map> leaveLecturerGroup(
    ...
    try {
      ...
      // Step 1: Get the lecturer group details
      Map<String, dynamic> formDataMap = {
        "uc_lecturer": ucLecturer,
      };

      await BaseClient.safeApiCall(
        Environment.getLecturerGroup, // URL untuk API
        RequestType.post, // Request Type adalah POST
        data: formDataMap, // Data yang dikirimkan dalam request
        headers: {}, // Header kosong jika perlu
        onSuccess: (response) async {
          if (response.data['status'] == true) {
            ...
            // Step 2: Request to leave the group
            formDataMap = {
              "uc_room": room,
              "uc_chat_user": chatUser['uc_chat_user'],
            };
            ...
            await BaseClient.safeApiCall(
              Environment.leaveGroup, // URL untuk API
              RequestType.post, // Request Type adalah POST
              data: formDataMap, // Data yang dikirimkan dalam request
              headers: {}, // Header kosong jika perlu
              onSuccess: (response) async {
              ...
              onError: (error) {
                log("Error leaving the group: ${error.message}");
                ret['status'] = false;
              },
            );
          }
        ...
        onError: (error) {
          log("Error fetching lecturer group: ${error.message}");
          ret['status'] = false;
        },
      );
    } on DioError catch (e) {
  ...
  static Future<Map> leaveGroup({required String uc_room, required db}) async {
    ...
    try {
      ...
      // Step 1: Prepare data for the leave group request
      Map<String, dynamic> formDataMap = {
        "uc_chat_user": userUc,
        "uc_room": uc_room,
      };
