...
class NewsCertificateController extends GetxController {
  ApiCallStatus apiCallStatus = ApiCallStatus.holding;
  var newsCertificateList = <NewsCertificate>[].obs;
  ...
  @override
  void onInit() {
    fetchNewsCertificate();
    super.onInit();
  }

  void loadPdfFromAssets() {
    final documentFuture = PdfDocument.openAsset('assets/data/demo.pdf');
  ...
  Future<void> fetchNewsCertificate() async {
    await BaseClient.safeApiCall(
      Environment.newsCertificate,
      RequestType.get,
      onLoading: () {
      ...
      onSuccess: (response) async {
        try {
          ...
          newsCertificateList.value = data.map((item) {
            return NewsCertificate.fromJson(item as Map<String, dynamic>);
          }).toList();
